    ...
    <>
      <div className="group flex flex-col justify-between bg-white dark:bg-slate-900 rounded-md shadow-md hover:shadow-lg shadow-slate-100 dark:shadow-slate-800 transition duration-500">
        <div className="p-3 pb-0 relative">
          ...
          >
            <Image
              src={`${env.NEXT_PUBLIC_API_URL}/${item.images[0]?.url}`}
              width={0}
              height={0}
              sizes="100vw"
          ...
          <div className="flex justify-between mt-3">
            {item.isShow && item.instructor && (
              <span className="flex items-center">
                <Image
                  src={`${env.NEXT_PUBLIC_API_URL}/${item.instructor.avatar}`}
                  width={32}
                  height={32}
                  className="size-8 rounded-full shadow shadow-slate-100 dark:shadow-slate-800 me-2"
