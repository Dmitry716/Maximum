  ...
  return (
    <>
      <section
        ...
        style={{
          backgroundImage: `url("${env.NEXT_PUBLIC_API_URL}/${course.images[0]?.url}")`,
        }}
      >
          ...
          <div className="grid grid-cols-1 ProseMirror  text-center mt-10">
            ...
            {course.isShow && (
              <div className="flex items-center mx-auto mt-8">
                <Image
                  src={`${env.NEXT_PUBLIC_API_URL}/${course.instructor?.avatar}`}
                  width={32}
                  height={32}
                  className="size-8 rounded-full shadow-md dark:shadow-gray-800"
                  alt=""
                />
                <h2 className="font-semibold text-white block ms-3">
                  {course.instructor?.name}
                </h2>
              </div>
            )}
            <span className="bg-violet-600 mt-8 text-white text-xs font-bold px-2.5 py-0.5 rounded w-fit mx-auto h-5">
              {Math.round(Number(course.price))} BYN/мес.
            </span>
        ...
        <div className="absolute text-center z-10 bottom-5 start-0 end-0 mx-3"></div>
      </section>
      ...
      <section className="relative md:py-24 py-16 overflow-hidden">
        ...
        <div className="container relative">
          ...
          <div className="grid md:grid-cols-2 grid-cols-1 mt-6 gap-6">
            {course.images[0] && (
              <Image
                src={`${env.NEXT_PUBLIC_API_URL}/${course.images[0]?.url}`}
                width={0}
                height={0}
                sizes="100vw"
            ...
            {course?.images[1] && (
              <div className="relative">
                <Image
                  src={`${env.NEXT_PUBLIC_API_URL}/${course.images[1]?.url}`}
                  width={0}
                  height={0}
                  sizes="100vw"
                  style={{ width: "100%", height: "auto" }}
        ...
        <div id="recent-courses" className="container relative md:mt-24 mt-16">
          ...
          <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 mt-6 gap-6">
            {courses &&
              courses
                .filter((item: Course) => item.id !== course.id)
                .slice(0, 3)
                .map((item: Course, index: number) => {
