...
export default function CoursesTwo() {
  ...
  return (
    <div className="grid lg:grid-cols-2 md:grid-cols-1  mt-6 gap-6">
      {courses &&
        courses.items.slice(0, 8).map((item: any, index: number) => (
          <div
            key={index}
            className="group grid md:grid-cols-2 justify-center md:justify-between bg-white dark:bg-slate-900 rounded-md shadow-md hover:shadow-lg shadow-slate-100 dark:shadow-slate-800 transition duration-500"
          >
            <div className="p-3 pb-0 relative">
              <Link
                href={`${env.NEXT_PUBLIC_SITE_URL}/courses/${item.url}`}
                className="relative overflow-hidden rounded-md"
              >
                <Image
                  src={
                    `${env.NEXT_PUBLIC_API_URL}/${item?.images[0]?.url}` ||
            ...
            <div className="p-3 flex flex-col justify-between">
              <div>
                ...
                <p className="text-slate-400 my-2 line-clamp-3">
                  {item.description}
                </p>
              </div>
              ...
              >
                Подробнее{" "}
                <i className="mdi mdi-arrow-right align-middle ms-1"></i>
              </Link>
            </div>
          </div>
        ))}
