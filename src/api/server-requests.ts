...
export async function getBlogByUrlServer(url: string): Promise<Blog> {
  try {
    ...
    console.error(
      `Full URL being called: ${env.NEXT_PUBLIC_API_URL}/api/blog/url/${url}`,
    );
...
export async function getSeoSettingsByPageNameServer(pageName: string) {
  try {
    ...
    if (!env.NEXT_PUBLIC_API_URL) {
      console.warn(
        `API URL not configured, skipping SEO data for page '${pageName}'`,
      );
      return null;
    }
    ...
    if (
      ...
      console.warn(
        `Invalid SEO response for page '${pageName}', using defaults.`,
      );
      return null;
    }
    ...
    if (
      ...
      console.warn(
        `SEO API not available during build for page '${pageName}', using defaults.`,
      );
      return null;
    }
