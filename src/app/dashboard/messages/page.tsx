...
export default function CoursesPage() {
  ...
  const { mutate: createOrUpdateApplication, isPending } = useMutation({
    ...
    onSuccess: (result: any) => {
      if (result) {
        toast.success(
          isEditing ? "Заявки обновлен успешно" : "Заявки создан успешно"
        );
        refetch();
        ...
        return (
          <div className="flex gap-2 justify-center">
            ...
            <Button
              variant="ghost"
              size="icon"
              onClick={() => handleDelete(applica.id)}
            >
      ...
      applications &&
        setApplications(
          applications.map((app) =>
            app.id === deleteTargetId ? { ...app, isLoading: true } : app
          )
        );
      const res = await deleteApplication(String(deleteTargetId));
      if (res) {
        applications &&
          setApplications(
            applications?.filter((course) => course.id !== deleteTargetId)
          );
      }
  ...
  return (
    <>
      ...
      >
        ...
        <Dialog
          open={moreDeatil.open}
          onOpenChange={(e) => setMoreDetail({ ...moreDeatil, open: e })}
        >
          <DialogContent className="max-w-xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="flex flex-col justify-center items-center gap-4 py-4">
              ...
              {moreDeatil.message && (
                <p className="text-lg">Cообщения: {moreDeatil.message}</p>
              )}
        ...
        >
          <DialogContent className="max-w-3xl max-h-[95vh] ">
            <DialogHeader className="px-2">
              <DialogTitle>
                {isEditing ? "Редактировать заявку" : "Создать новую заявку"}
              </DialogTitle>
              <DialogDescription>
                {isEditing
                  ? "Измените информацию о заявке здесь."
                  : "Заполните информацию о новой заявке здесь."}
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleSubmit(onSubmit)}>
              <div
                className="grid gap-4 px-2 py-4 overflow-y-auto"
              ...
              >
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    {watch("photo") && (
                      <div className="relative h-40 w-full max-w-md rounded-md overflow-hidden border">
                        <Image
                          src={
                            `${process.env.NEXT_PUBLIC_API_URL}/${watch(
                              "photo"
                            )}` || "/placeholder.svg"
                          }
                          alt="Предпросмотр"
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value={ApplicationStatus.NEW}>
                        Новая
                      </SelectItem>
                      ...
                      <SelectItem value={ApplicationStatus.CONFIRMED}>
                        Завершена
                      </SelectItem>
                      <SelectItem value={ApplicationStatus.CANCELLED}>
                        Отменена
                      </SelectItem>
                    </SelectContent>
                  </Select>
              ...
              <DialogFooter className="pt-4">
                <Button type="submit" disabled={!isDirty || isPending}>
                  {isEditing ? "Сохранить изменения" : "Создать сообщение"}
                </Button>
              </DialogFooter>
            </form>
          </DialogContent>
