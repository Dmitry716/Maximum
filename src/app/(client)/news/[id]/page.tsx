...
export async function generateStaticParams() {
  const blogs = await getNews(1, 1000, 'published') as { items: NewsItem[], total: number };
...
export default async function Page({ params }: { params: paramsType }) {
  ...
  const relatedPosts = await getNews(1, 3, 'published', blog.category, blog.id) as { items: NewsItem[], total: number };

  const latestPosts = await getNews(1, 5, 'published') as { items: NewsItem[], total: number };
        ...
        <div className="container relative">
          <div className="grid ProseMirror grid-cols-1 text-center mt-10">
            ...
            <ul className="list-none mt-6">
              ...
              <li className="inline-block text-white/50 mx-5"> <span className="text-white block">
                Дата:</span> <span className="block">{format(blog.date, "d MMMM yyyy", { locale: ru })}</span></li>
          ...
          <div className="grid lg:grid-cols-12 md:grid-cols-2 grid-cols-1 gap-6">
            <div className="lg:col-span-8 md:order-1 order-2">
              <div className="relative lg:p-6 md:p-4 p-2 overflow-hidden rounded-xl shadow dark:shadow-gray-800">
                <Image src={`${process.env.NEXT_PUBLIC_API_URL}/${blog?.image}`} width={0} height={0} sizes='100vw' style={{ width: '100%', height: 'auto' }} alt="" />
                {blog?.content && <RenderNovel contentFromDB={blog?.content} />}
              </div>
            </div>

            {latestPosts.items.filter((post) => post.id !== blog.id).length > 0
              && <div className="lg:col-span-4 md:order-2 order-1">
                {latestPosts && <BlogsSidebar title={"Недавние посты"} blogRecentPost={latestPosts.items.filter((post) => post.id !== blog.id)} />}
              </div>}
          </div>
        </div>

        {relatedPosts?.items && relatedPosts?.items?.filter((post) => post.id !== blog.id).length > 0 && <div className="container relative md:mt-24 mt-16">
          <div className="grid grid-cols-1 pb-6 text-center">
            <h4 className="mb-6 md:text-3xl text-2xl md:leading-normal leading-normal font-semibold">Связанные новости</h4>
